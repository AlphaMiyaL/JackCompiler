...
/**
 * @author Brian Song
 * @description gets its input from a JackTokenizer and writes output to an output file. The output
 *              is generated by a series of compile routines, each designed to handle the
 *              compilation of a specific Jack language construct.
 */
public class CompilationEngine {
	...
	/**
	 * Compiles a parameter list that can possibly be empty. Does not handle enclosing parenthesis
	 * tokens
	 * 
	...
	/**
	 * Compile sequence of statements Does not handle enclosing curly brackets tokens
	 */
	...
	/**
	 * Compiles and prints compiled expression
	 * 
	 * @return 1 if an expression was compiled, 0 otherwise
	 */
	...
	/**
	 * Compiles a term If current token is an identifier, routine must resolve into a variable, array
	 * element, or subroutine call Any other tokens are not part of this term and are not advanced over
	 */
	...
	/**
	 * Compiles a possibly empty list of comma seperated list of expressions prints compiled expression
	 * list and returns
	 * 
	 * @return how many expressions were compiled
	 */
	...
	private String getType() {
		if(jt.tokenType()==JackTokenizer.KEYWORD && (jt.keyword().equalsIgnoreCase("int") || jt.keyword().equalsIgnoreCase("char")
		        || jt.keyword().equalsIgnoreCase("boolean") || jt.keyword().equalsIgnoreCase("void"))) {
			return getKeyword();
		}
		else if(jt.tokenType()==JackTokenizer.IDENTIFIER) {
	...
	private boolean isOperator() {
		if(jt.tokenType()==JackTokenizer.SYMBOL) {
			return(jt.symbol()=='+' || jt.symbol()=='-' || jt.symbol()=='*' || jt.symbol()=='/' || jt.symbol()=='&' || jt.symbol()=='|'
	...
	private boolean isKeywordConstant() {
		if(jt.tokenType()==JackTokenizer.KEYWORD) {
			return jt.keyword().equalsIgnoreCase("true") || jt.keyword().equalsIgnoreCase("false") || jt.keyword().equalsIgnoreCase("null")
	...
	private boolean isStartOfTerm() {
		if(jt.tokenType()==JackTokenizer.INT_CONST)
			return true;
		if(jt.tokenType()==JackTokenizer.STRING_CONST)
			return true;
